...
from persistence.models import Station, Measurement
from persistence.database import init_db, db_session
from persistence.settings import DB_FILE
...
with io.open(tmp(DWD_STATIONS), mode='r', encoding=enc) as stations_file:
    ...
    while line:
        ...
        if id5 in published_id5s:
                    ...
                    if re.match(r'^produkt_klima_tag_.*\.txt$', info.filename):
                        ...
                        with io.open(tmp(info.filename), mode='r') as klima_file:
                            ...
                            while kline:
                                ...
                                measurement = Measurement(station_id=station_id,
            ...
            log.info("ommitting nonpublished id5: " + id5)
        line = stations_file.readline()
        progress_count += 1
        if progress_count % 100:
            log.info(progress_count + ' files extracted')
db_session.commit()
